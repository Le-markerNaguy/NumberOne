...
export default function AdminsPage() {
  ...
  if (role !== "superadmin") {
    return (
      <div className="flex flex-col items-center justify-center h-64">
        ...
        <p className="text-gray-500">Seul le super administrateur peut acceder a cette page.</p>
      </div>
    )
  }
  ...
  const createAdmin = async () => {
    ...
    if (res.success && res.data) {
      ...
      setShowCreate(false)
      toast({ title: "Succes", description: "Administrateur cree avec succes" })
      setForm({ nom: "", email: "", mot_de_passe: "", role_id: "" })
    } else {
      toast({ title: "Erreur", description: res.error || "Impossible de creer l'admin" })
    }
  }
  ...
  const copyEmail = (email: string) => {
    ...
    toast({ title: "Copie", description: "Email copie dans le presse-papiers" })
  }
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8">
        <div>
          <h1 className="text-2xl lg:text-3xl font-bold text-gray-900">Gestion des administrateurs</h1>
          <p className="text-gray-500">Creer, modifier ou supprimer des administrateurs et leur role</p>
        </div>
        ...
        <Card className="mb-6">
          ...
          <CardContent>
            ...
            <div className="flex items-center gap-2 mt-4">
              <Button className="bg-orange-500 hover:bg-orange-600 text-white" onClick={createAdmin}>
                Creer
              </Button>
        ...
        <CardContent>
          {loading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {admins.map((admin) => (
                <div key={admin.id} className="p-4 bg-gray-50 rounded-lg flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-orange-500 capitalize">{admin.role || "admin"}</div>
                    </div>
                  </div>
                  <div className="flex items-center gap-1">
                    <Button size="icon" variant="ghost" className="h-8 w-8" onClick={() => copyEmail(admin.email)}>
